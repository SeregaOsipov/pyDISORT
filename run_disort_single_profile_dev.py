...
atm_ds = get_atmospheric_profile()
atm_ds = atm_ds.sel(lat=lats, lon=lons, time=date, method='nearest')  # TODO: date selection should be controlled by tollerance
...
atm_ds.z[0,0,0] = 0  # TODO: temp test is z has to start from zero
atm_ds = atm_ds.isel(level=range(len(atm_ds.level)-2))
...
gases_ds = gases_ds.interp(level=atm_ds.level.data, kwargs={"fill_value": "extrapolate"})  # TODO: extrapolating blindly is bad
...
h2o_ds.const[:] = atm_ds.r[:].data  # units H
